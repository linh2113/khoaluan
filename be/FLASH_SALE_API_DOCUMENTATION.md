    ...
    <div className="product-card">
      ...
      {hasDiscount && product.discountInfo && (
        <div className="discount-info">
          ...
          <span className="discount-end">Kết thúc: {new Date(product.discountInfo.endDate).toLocaleDateString()}</span>
        </div>
      )}
    </div>
  );
...
const validateDiscountForm = (formData) => {
  ...
  if (formData.type === 'CATEGORY' && (!formData.categoryIds || formData.categoryIds.length === 0)) {
